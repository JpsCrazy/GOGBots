...
def ProfileChecker(User, BodyText, RequestOrComment, PostID, GoG):
    try:
        import re
        RemovalReason = None
        UserIDSearch = re.search(r"(?i)((https{0,1}://){0,1}(www\.){0,1}(steamcommunity\.com\/(profiles|id)\/[^!\(\)\[\]/\s]*|gog\.com\/u\/[^!\(\)\[\]/\\\s]*|my\.playstation\.com/profile/[^!\(\)\[\]/\\\s]*|psnprofiles\.com/[^!\(\)\[\]/\\\s]*|\/profile\?gamertag=[^!\(\)\[\]/\\\s]*|mygamerprofile\.net[^!\(\)\[\]/\\\s]*|xboxgamertag\.com\/search\/[^!\(\)\[\]/\\\s]*|(SW){0,1}-{0,1} {0,1}([0-9]{4}-){2}[0-9]{4}|(?!https:\/\/\S*?\.itch\.io\/[A-Z,0-9])https:\/\/\S*?\.itch\.io))", str(BodyText))
        if UserIDSearch is not None:
...
#                         print(ID64)
                        ...
                        if ID64 != "None":
                            ...
                            if ID64 != "None":
                                ...
                                print("ID64 converter broken; skipping", UserID, ID64Backup)
                else:
